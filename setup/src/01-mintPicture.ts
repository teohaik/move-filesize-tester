...
const mintPicAndUploadData = async () => {
    ...
    const picture = tx.moveCall({
        target: `${PACKAGE_ADDRESS}::filesize::mint_picture`,
        arguments: [
        ...
        if (status === "success") {
            ...
            res?.objectChanges?.find((obj) => {
                if (obj.type === "created" && obj.objectType.endsWith("filesize::Picture")) {
                    const objIdString = `PIC_ID=${obj.objectId}\n`;
                    console.log("\r \n Image Object Created!  ID = ", objIdString);

                    updatePicData(obj.objectId);
                }
            });
...
const updatePicData = async (picId : string) => {

    console.log("Uploading more bytes for picture ID = ", picId);
    ...
    while (endByte < imageData.length) {
        ...
        tx.moveCall({
            target: `${PACKAGE_ADDRESS}::filesize::add_bytes_in_picture`,
            arguments: [
                tx.object(picId),
                tx.pure(Array.from(imageData.slice(startByte, endByte)), "vector<u8>"),
            ],
